...
const PregledDetail = (props) => {
    const {
        closeNoviPregled, pregledId, refreshExaminations,
        backendRoute, bearerToken, user, examination, handleLogOut, currentRole
    } = props
        ...
        <>
            <div id="addPatientsWrapper" className="shadow-lg">
                <div id="addPatientsInner">
                    <h5 className="pt-3 px-4 mt-2 mb-3 " style={{textAlign: "left"}}>Detalji
                        pregleda {/* <button className='btn btn-tertiary mt-1' style={{float: 'right'}}>Povijest </button>  */}
                            ...
                            <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gridColumnGap: "20px"}}
                                ...
                                <div className="mb-3">
                                    <label htmlFor="username" className=" col-12 text-label"
                                           style={{float: 'left', textAlign: "left"}}>Pacijent</label>
                                    <p style={{textAlign: "left"}}
                                       className="text"> {examination.patient.first_name + " " + examination.patient.last_name} </p>
                                </div>
                                <div className="mb-3">
                                    <label htmlFor="username" className=" col-12 text-label"
                                           style={{float: 'left', textAlign: "left"}}>Doktor</label>
                                    <p style={{textAlign: "left"}}
                                       className="text">{examination.doctor.first_name + " " + examination.doctor.last_name}</p>
                                </div>
                                <div className="mb-3">
                                    <label htmlFor="username" className=" col-12 text-label"
                                           style={{float: 'left', textAlign: "left"}}>Datum</label>
                                    <p style={{textAlign: "left"}}
                                       className="text"> {examination.date.substring(0, 10)}  </p>
                                </div>
                                <div className="mb-3">
                                    <label htmlFor="username" className=" col-12 text-label"
                                           style={{float: 'left', textAlign: "left"}}>Lokacija</label>
                                    <p style={{textAlign: "left"}}
                                       className="text"> {examination.address.country + ", " + examination.address.city + ", " + examination.address.street + ", " + examination.address.number} </p>
                                </div>

                                {currentRole === 'doctor' || currentRole === 'pediatrician' || currentRole === 'admin' ? (
                                    <div className="mb-3">
                                        <label htmlFor="username" className=" col-12 text-label"
                                               style={{float: 'left', textAlign: "left"}}>Opis pregleda</label>
                                        {/*<p style={{textAlign:"left"}} className ="text">{insertNewline(examination.report)}</p>*/}
                                        <textarea rows="7" type="text" className="form-control"
